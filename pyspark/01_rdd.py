...
rdd= sc.parallelize([1,2,3,4,5,6,7], numSlices = 3)
rdd2 = sc.parallelize([[1, 2], [1, 3], [2, 4]])

# 转换函数
# map 针对rdd里面的每一个元素执行操作
rdd12 = rdd.map(lambda x: x ** 2)

# filter
rdd13 = rdd.filter(lambda x: x > 3)

# flatmap 先映射后扁平
rdd14 = rdd.flatMap(lambda x: (x, 1))

# mapPartition 独立的在rdd的每一个分片上运行 func的函数类型必须是Iterator[T] => Iterator[U]
rdd15 = rdd.mapPartitions()

# sample 抽样
rdd16 = rdd.sample(withReplacement = True, fraction = 0.5, seed = 1234)

# union 合并两个rdd
rdd1 = sc.parallelize([10, 20, 30])
rdd17 = rdd.union(rdd1)
